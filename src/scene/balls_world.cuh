...
__device__ void random_balls_world(hittable_list *world) {
    printf("world size: %u, idx : %u, begin : %llu\n", world->size, world->idx, world->list);
    ...
    for (int a = -11; a < 11; a++) {
        for (int b = -11; b < 11; b++) {
            ...
            point3 center(a + 0.9f * curand_uniform(rand_state), 0.2f, b + 0.9f * curand_uniform(rand_state));
...
//    printf("box : %d %d\n",box->idx, box->size);
    ...
    for (uint32_t i = 0; i < box->idx; i++) {
        printf("ball%d: %.5f %.5f %.5f\n", i, ((sphere *) box->list[i])->center[0],
               ((sphere *) box->list[i])->center[1], ((sphere *) box->list[i])->center[2]);
    }
}

//__global__ void random_balls_world(hittable_list *world) {
...
//    for (int a = -11; a < 11; a++) {
//        for (int b = -11; b < 11; b++) {
//            auto choose_mat = host::random_float();
//            point3 center(a + 0.9f * host::random_float(), 0.2f, b + 0.9f * host::random_float());
//
//            if ((center - point3(4, 0.2, 0)).length() > 0.9) {
...
//                if (choose_mat < 0.8) {
//                    // diffuse
//                    auto albedo = random_vec3f() * random_vec3f();
...
//                    auto fuzz = host::random_float(0, 0.5)();
